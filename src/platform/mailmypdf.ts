...
export interface MailMyPDFDocument { id: string; filename: string; mime_type: string; sha256: string; size_bytes: number; source?: string; created_at: string; }
export interface DisputeRecipient { name: string; address_line1: string; address_line2?: string | null; city: string; state: string; postal_code: string; country?: string; }
export interface CreateDisputeCommunicationInput { document_id: string; recipient: DisputeRecipient; mail_type: MailType; matter_reference: string; matter_type: string; metadata?: Record<string, unknown>; idempotency_key: string; }
...
export async function createCommunication(input: CreateDisputeCommunicationInput): Promise<MailMyPDFCommunication> {
