...
export const disputeFindingSchema = z.object({
  id: z.string(), state: disputeFindingStateSchema, title: z.string(), detail: z.string(), sourceExcerpt: z.string().optional(), severity: z.enum(["high", "medium", "low"]),
});
export type DisputeFinding = z.infer<typeof disputeFindingSchema>;

export const evidenceItemSchema = z.object({
  id: z.string(), description: z.string(), status: z.enum(["missing", "requested", "provided", "verified", "rejected", "not_applicable"]), supportsFindingIds: z.array(z.string()).default([]),
});
...
export function analyzeDisputeWorkflowInput(input: { documentId: string; text: string; profile: WorkflowProfile; workflowFacts?: Record<string, string | undefined>; objective?: string }): DisputeAnalysis {
  ...
  for (const requirement of input.profile.evidenceRequirements) evidence.push({ id: `evidence-${slugify(requirement)}`, description: requirement, status: "requested", supportsFindingIds: [] });
...
export function analyzeCreditReportInput(input: { documentId: string; text: string; bureau?: string; accountNumber?: string; reportDate?: string; errorType?: string; facts?: string; objective?: string }): DisputeAnalysis {
  ...
  const normalizedFacts = [input.bureau ? { label: "credit_bureau", value: input.bureau } : null, input.accountNumber ? { label: "account_reference", value: input.accountNumber } : null, input.reportDate ? { label: "report_date", value: input.reportDate } : null, input.errorType ? { label: "error_type", value: input.errorType } : null].filter((value): value is { label: string; value: string } => Boolean(value));
...
/** Pre-mail gate: proof does not exist yet and must not be a prerequisite for submission. */
...
/** Post-mail gate: tracking/proof are reconciled after the provider accepts the communication. */
export function canCompleteDisputeProof(params: { trackingNumber: string | null; proofReady: boolean }): boolean {
  return Boolean(params.trackingNumber) && params.proofReady;
}

/** Backward-compatible helper for callers that have a complete lifecycle state. */
