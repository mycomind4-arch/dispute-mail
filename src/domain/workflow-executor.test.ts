...
describe("profile-driven dispute workflow executor", () => {
  ...
  it("requires explicit approval, payment, provider submission, tracking, and proof", () => {
    const input = {
      workflowId: "credit-report" as const,
      documentId: "credit-doc",
      text: "Credit report source text",
      facts: {
        creditBureau: "Equifax",
        accountReference: "ABC123",
        reportingError: "Balance is incorrect",
        requestedCorrection: "Correct the balance",
      },
      objective: "Correct the reported balance.",
    };

    const withoutApproval = runProfiledDisputeWorkflow(input, {
      draftValidated: true,
      humanApproved: false,
    ...
    const withApprovalButNoProof = runProfiledDisputeWorkflow(input, {
      draftValidated: true,
      humanApproved: true,
      recipientComplete: true,
      paymentComplete: true,
