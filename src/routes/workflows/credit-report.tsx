...
import { ArrowLeft, ArrowRight, FileUp, ShieldAlert, CheckCircle2, Mail, Clock, PackageCheck, Stamp, CreditCard, Check, Lock } from "lucide-react";
...
function CreditReport() {
  ...
  const [submissionStatus, setSubmissionStatus] = useState<"not_ready" | "blocked">("not_ready");
...
[Your SSN (last 4 digits)]`;
  ...
  function canContinue() {
    switch (step) {
      ...
      case 7: return Boolean(recipient.name && recipient.address1 && recipient.city && recipient.state && recipient.zip);
      case 9: return false; // No live provider/submission integration is installed in this route.
      default: return true;
    }
  }
      ...
      <div className="container py-8 md:py-12">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="card p-6 md:p-10">
            ...
            {step === 9 && (<><div className="eyebrow">9 · Checkout</div><h2 className="mt-3 text-2xl font-bold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Review and pay</h2><div className="mt-6 space-y-3"><div className="flex items-center justify-between rounded-lg border border-warm-border px-4 py-3 text-sm"><span className="text-slate-500">Mail type</span><span className="font-semibold text-teal-700">{mailOptions.find((m) => m.id === mailType)?.label}</span></div><div className="flex items-center justify-between rounded-lg border border-warm-border px-4 py-3 text-sm"><span className="text-slate-500">Recipient</span><span className="font-semibold text-teal-700">{recipient.name || "—"}</span></div><div className="flex items-center justify-between rounded-lg border border-warm-border px-4 py-3 text-sm"><span className="text-slate-500">Total</span><span className="text-lg font-bold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>{mailOptions.find((m) => m.id === mailType)?.price}</span></div></div><div className="alert alert-warning mt-4"><Lock size={16} className="shrink-0" /> Live payment and physical-mail submission are not wired into this workflow yet. No payment or mailing will be initiated from this page.</div>{submissionStatus === "blocked" && <div className="mt-3 text-sm font-semibold text-slate-500">Submission is blocked until the authenticated payment, fulfillment, tracking, and proof path is installed.</div>}</>)}
            {step === 10 && (<div className="text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-rose-50"><CheckCircle2 size={32} className="text-rose-500" /></div><h2 className="mt-5 text-2xl font-bold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Workflow complete</h2><p className="mt-3 text-slate-400">The draft workflow has completed preparation and review. No physical mailing was initiated by this UI.</p><div className="mt-6 inline-flex items-center gap-2 rounded-xl border border-warm-border px-4 py-3 text-sm"><PackageCheck size={16} className="text-rose-500" /><span className="text-slate-500">Mailing status:</span><span className="font-semibold text-teal-700">Not submitted</span></div><div className="mt-8 flex justify-center gap-3"><Link to="/" className="btn-outline">Back to home</Link><Link to="/workflows/credit-report" className="btn-primary">Start another</Link></div></div>)}

            {step < 10 && (<div className="mt-8 flex items-center justify-between"><button onClick={back} disabled={step === 0} className="btn-ghost disabled:opacity-30"><ArrowLeft size={16} /> Back</button><button onClick={next} disabled={!canContinue()} className="btn-primary">{step === 9 ? "Payment + mailing integration required" : "Continue"} <ArrowRight size={16} /></button></div>)}
          </div>
