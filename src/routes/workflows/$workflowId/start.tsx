...
function WorkflowIntakePage() {
  ...
  const submit = async () => {
    if (!selectedFile) {
      setError("Choose the document you want Claude to analyze.");
      return;
    }
    setRunning(true);
    setError(null);
  ...
  return (
    <main className="min-h-screen bg-cream">
      <SiteHeader />
      <section className="border-b border-warm-border bg-teal-50 py-12">
        <div className="container max-w-4xl">
          <Link to="/workflows/$workflowId" params={{ workflowId }} className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500"><ArrowLeft size={16} /> Back to workflow</Link>
          <div className="eyebrow mt-7">START WORKFLOW</div>
          <h1 className="mt-2 text-4xl font-bold text-teal-700 md:text-5xl" style={{ fontFamily: "var(--font-serif)" }}>{profile.primaryKeyword}</h1>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-500">{profile.problem}</p>
        </div>
      </section>
        ...
        <div className="space-y-7">
          <div className="card p-7">
            <div className="flex items-start gap-3">
              <FileSearch className="mt-1 text-teal-700" size={22} />
              <div><h2 className="text-xl font-semibold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Upload the document</h2><p className="mt-1 text-sm leading-6 text-slate-500">Upload the actual PDF. The server stores it in MailMyPDF, then sends the document itself to Claude for extraction and analysis.</p></div>
            </div>
            <input ref={fileInputRef} type="file" accept="application/pdf" className="sr-only" onChange={(event) => { const file = event.target.files?.[0] ?? null; setSelectedFile(file); setResult(null); setError(null); }} />
            <button type="button" onClick={() => fileInputRef.current?.click()} className="mt-6 flex w-full items-center justify-center gap-3 rounded-xl border-2 border-dashed border-warm-border bg-white px-5 py-10 text-sm font-semibold text-teal-700 transition hover:border-teal-500">
              <UploadCloud size={24} /> {selectedFile ? selectedFile.name : "Choose a PDF document"}
            </button>
            {selectedFile && <p className="mt-3 text-xs text-slate-400">{(selectedFile.size / 1024 / 1024).toFixed(2)} MB · PDF · ready for server-side analysis</p>}
          </div>

          <div className="card p-7">
            <h2 className="text-xl font-semibold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Problem-specific facts</h2>
            <div className="mt-5 space-y-5">{factFields.map(({ label, key }) => <label key={key} className="block"><span className="text-sm font-semibold text-teal-700">{label}</span><input value={facts[key] ?? ""} onChange={(e) => setFacts((current) => ({ ...current, [key]: e.target.value }))} className="mt-2 w-full rounded-xl border border-warm-border bg-white p-3 text-sm text-slate-700 outline-none focus:border-teal-500" /></label>)}</div>
          </div>

          <div className="card p-7">
            <h2 className="text-xl font-semibold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Evidence review</h2>
            <p className="mt-1 text-sm leading-6 text-slate-500">These statuses are recorded separately from the AI analysis. Human verification remains a required gate.</p>
            <div className="mt-5 space-y-4">{evidenceItems.map((item) => <div key={item.id} className="rounded-xl border border-warm-border bg-white p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-1 text-teal-700" size={19} /><div className="min-w-0 flex-1"><div className="text-sm font-semibold text-teal-700">{item.description}</div><select value={evidenceStatuses[item.id] ?? "requested"} onChange={(e) => setEvidenceStatuses((current) => ({ ...current, [item.id]: e.target.value as EvidenceItem["status"] }))} className="mt-3 w-full rounded-lg border border-warm-border bg-white p-2 text-sm text-slate-600"><option value="requested">Requested — not yet supplied</option><option value="provided">Provided — needs verification</option><option value="verified">Verified — reviewed by a human</option><option value="not_applicable">Not applicable — explicitly reviewed</option></select></div></div></div>)}</div>
          </div>

          <div className="card p-7">
            <h2 className="text-xl font-semibold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Requested outcome</h2>
            <p className="mt-1 text-sm leading-6 text-slate-500">{profile.objectivePrompt}</p>
            <textarea value={objective} onChange={(e) => setObjective(e.target.value)} rows={5} className="mt-4 w-full rounded-xl border border-warm-border bg-white p-4 text-sm leading-6 text-slate-700 outline-none focus:border-teal-500" placeholder="Describe what you want the recipient to investigate, correct, stop, or otherwise address." />
          </div>
