...
import { ArrowLeft, CheckCircle2, FileSearch, Loader2, ShieldCheck } from "lucide-react";
...
function WorkflowIntakePage() {
  ...
  const run = () => {
    setRunning(true);
    const execution = runProfiledDisputeWorkflow({ workflowId, documentId: `web-${Date.now()}`, text, facts, evidenceStatuses, objective });
    setResult(execution);
    setRunning(false);
  };
    ...
    <main className="min-h-screen bg-cream">
      ...
      <section className="container grid gap-8 py-12 lg:grid-cols-[1fr_340px]">
        <div className="space-y-7">
          <div className="card p-7"><div className="flex items-start gap-3"><FileSearch className="mt-1 text-teal-700" size={22} /><div><h2 className="text-xl font-semibold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Source document</h2><p className="mt-1 text-sm leading-6 text-slate-500">Paste extracted text for this prototype intake. Production upload/storage remains a deployment integration boundary.</p></div></div><textarea value={text} onChange={(e) => setText(e.target.value)} rows={10} className="mt-5 w-full rounded-xl border border-warm-border bg-white p-4 text-sm leading-6 text-slate-700 outline-none focus:border-teal-500" placeholder="Paste the relevant notice, statement, report, or correspondence text here." /></div>
          ...
          <div className="card p-7"><h2 className="text-xl font-semibold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>Evidence review</h2><p className="mt-1 text-sm leading-6 text-slate-500">Mark evidence only after the corresponding source has actually been supplied and reviewed. The production workflow will require persistent evidence records before approval.</p><div className="mt-5 space-y-4">{evidenceItems.map((item) => <div key={item.id} className="rounded-xl border border-warm-border bg-white p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-1 text-teal-700" size={19} /><div className="min-w-0 flex-1"><div className="text-sm font-semibold text-teal-700">{item.description}</div><select value={evidenceStatuses[item.id] ?? "requested"} onChange={(e) => setEvidenceStatuses((current) => ({ ...current, [item.id]: e.target.value as EvidenceItem["status"] }))} className="mt-3 w-full rounded-lg border border-warm-border bg-white p-2 text-sm text-slate-600"><option value="requested">Requested — not yet supplied</option><option value="provided">Provided — needs verification</option><option value="verified">Verified — reviewed by a human</option><option value="not_applicable">Not applicable — explicitly reviewed</option></select></div></div></div>)}</div></div>
