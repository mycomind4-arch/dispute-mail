...
function AuthPage() {
  ...
  return <main className="min-h-screen bg-cream"><SiteHeader /><section className="py-12 md:py-20"><div className="container max-w-4xl"><div className="grid overflow-hidden rounded-2xl border border-warm-border md:grid-cols-2"><div className="p-8 md:p-10" style={{ background: "linear-gradient(135deg, #0f766e 0%, #134e4a 100%)" }}><div className="flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/15"><FileWarning size={18} className="text-rose-400" /></div><span className="text-lg font-bold text-white" style={{ fontFamily: "var(--font-serif)" }}>Dispute Mail</span></div><h1 className="mt-8 text-2xl font-bold text-white" style={{ fontFamily: "var(--font-serif)" }}>Your disputes, organized and sent.</h1><p className="mt-4 text-sm leading-7 text-white/60">Use your MailMyPDF Account to save dispute drafts, track mailings, and keep proof of submission.</p><ul className="mt-8 space-y-3">{["Save and resume workflows", "Track mailings", "Keep proof of submission", "Reuse recipient addresses"].map((item) => <li key={item} className="flex items-center gap-2 text-sm text-white/70"><CheckCircle2 size={16} className="text-rose-400" /> {item}</li>)}</ul></div><div className="flex flex-col justify-center bg-white p-8 md:p-10"><h2 className="text-xl font-bold text-teal-700" style={{ fontFamily: "var(--font-serif)" }}>{signUp ? "Create your MailMyPDF Account" : "Welcome back"}</h2><p className="mt-2 text-sm text-slate-400">{magic ? "We'll email you a secure sign-in link." : "Sign in to save drafts and submit dispute mailings."}</p>{error && <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700">{error}</div>}{notice && <div className="mt-4 rounded-lg border border-teal-200 bg-teal-50 p-3 text-xs text-teal-800">{notice}</div>}<form onSubmit={submit} className="mt-6 flex flex-col gap-4"><div><label className="input-label">Email address</label><input className="input-field" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></div>{!magic && <div><label className="input-label">Password</label><input className="input-field" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></div>}<button className="btn-rose mt-2 w-full justify-center" disabled={busy || loading}>{busy ? "Working…" : magic ? "Email me a sign-in link" : signUp ? "Create account" : "Sign in"} <ArrowRight size={16} /></button></form><div className="mt-5 flex flex-wrap justify-center gap-4 text-xs text-slate-400"><button type="button" className="text-rose-600 underline" onClick={() => { setMagic(!magic); setError(null); setNotice(null); }}>{magic ? "Use password" : "Use magic link"}</button>{!signUp && !magic && <button type="button" className="text-rose-600 underline" onClick={reset}>Forgot password?</button>}<button type="button" className="text-rose-600 underline" onClick={() => { setSignUp(!signUp); setMagic(false); setError(null); setNotice(null); }}>{signUp ? "Already have an account? Sign in" : "Create an account"}</button></div><p className="mt-5 text-center text-xs text-slate-300"><Link to="/terms" className="text-rose-600 underline">Terms</Link> · <Link to="/privacy" className="text-rose-600 underline">Privacy</Link></p></div></div></div></section><SiteFooter /></main>;
}
